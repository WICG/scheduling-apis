...
This section describes the interfaces, definitions, and algorithms used for
scheduling tasks.
...
The `Scheduler` Interface {#sec-scheduling-tasks-scheduler}
---------------------
...
### Scheduler Task Sources ### {#sec-defscheduler-task-sources}
...
<div algorithm="schedule a postTask task">
  ...
  1. If |delay| is equal to 0 then
    1. [=schedule a task to invoke a callback=] for |scheduler| given |queue|, |signal|, |callback|, and |p|.
    1. Return p.
...
<div algorithm="select the task queue of the next scheduler task">
  ...
  1. If |candidate queue| is null, return null; otherwise, return |candidate queue|'s [=scheduler task queue/tasks=].
